...
use core::ffi::CStr;
use usys::{chmod, creat, open, stat, unlink, IoRead};
...
fn main() -> ! {
    ...
    match creat(path, 0o644) {
        Ok(fd) => {
            usys::println!("V Created test.txt (fd={})", fd.0);
            ...
            match fd.write(data) {
                Ok(n) => {
                    usys::println!("V Wrote {} bytes", n);
                }
                Err(_) => {
                    usys::println!("X Failed to write");
                }
            }
        ...
        Err(_) => {
            usys::println!("X Failed to create test.txt");
        }
    }
    ...
    match open(path) {
        Ok(fd) => {
            usys::println!("V Opened test.txt (fd={})", fd.0);

            let mut buf = [0u8; 128];
            match fd.read(&mut buf) {
                Ok(n) => {
                    usys::println!("V Read {} bytes:", n);
                    if let Ok(s) = core::str::from_utf8(&buf[..n]) {
                        usys::println!("  Content: {}", s);
                    }
                }
                Err(_) => {
                    usys::println!("X Failed to read");
                }
            }
    ...
    match chmod(path, 0o400) {
        Ok(_) => {
            usys::println!("V chmod() succeeded");
    ...
    match unlink(path) {
        Ok(_) => {
            usys::println!("V File deleted");

            // Verify it's gone
            match stat(path, &mut stat_buf) {
                Ok(_) => {
                    usys::println!("X File still exists!");
                }
                Err(_) => {
                    usys::println!("V File no longer exists");
                }
            }
        }
        Err(_) => {
            usys::println!("X unlink() failed");
        }
    }
    ...
    match creat(path, 0o644) {
            ...
            match open(path) {
                Ok(fd) => {
                    ...
                    if let Ok(n) = fd.read(&mut buf) {
                        usys::println!("V Multiple writes successful:");
                        if let Ok(s) = core::str::from_utf8(&buf[..n]) {
                            usys::println!("{}", s);
                        }
                    }
        ...
        Err(_) => {
            usys::println!("X Failed to create file");
        }
    }
